#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#
#      Grid the buffer zones around the site polygons       #
#  and calculate percentage overlap with raster grid cells  #
...
#-#-# Load libraries #-#-#
packList <- c("maptools","raster","ggplot2","rgdal","snowfall","rgeos","sf")
...
## Read load shapefile
PA.map <- readOGR("Combined_data_global_IUCN_WHS_KBA_countries.shp", encoding="UTF-8")
head(PA.map@data)
IUCNPoly <- PA.map
...
r.grid.l <- raster(nrows=286, ncols=719, xmn=xmin, xmx=xmax, ymn=ymin, ymx=ymax,crs="+proj=longlat +ellps=WGS84 +towgs84=0,0,0,0,0,0,0 +no_defs +datum=WGS84")
...
gridIUCN <- lapply(1:length(slot(IUCNPoly, "polygons")),function(s){
  ...
  ## Derive buffer around site
  IUCNSPDF_B <- gConvexHull(IUCNSPDF) # clean errors in buffer polygon and joins fragmented PAs - careful buffer increases depending on shape of site
  IUCNSPDFBuf <- buffer(IUCNSPDF_B, width=0.5) # define buffer around site (0.5 degrees note polygons are treated different for raster its in meters)
  IUCNSPDFBufOut <- rgeos::gDifference(IUCNSPDFBuf, IUCNSPDF, byid=TRUE) # keep only the buffer zone and discard the site
  ...
  r.grid.l <- raster(nrows=nrow, ncols=ncol, xmn=xmin.pa, xmx=xmax.pa, ymn=ymin.pa, ymx=ymax.pa,crs="+proj=longlat +ellps=WGS84 +towgs84=0,0,0,0,0,0,0 +no_defs +datum=WGS84")
...
save(gridIUCNnoNull,file="Combined_data_global_IUCN_WHS_KBA_grid_country_Buffers.Rdata",compress="xz")
head(gridIUCNnoNull)
str(gridIUCNnoNull)
